# 三维血管导航渲染改造方案

> 目标：把当前 Godot 3D 血管视图从“低模发光管道总览”升级为接近参考图的“医学影像式玻璃血管导航视图”。本文只讨论血管建模、材质、相机与叠加层，不包含图中中文文字箭头标注。

## 一、当前结论

参考图的核心观感不是简单的蓝色发光，而是：

- 血管几何连续圆滑，分叉处自然过渡。
- 主血管近景清晰，远端分支虚化、变暗、退入背景。
- 管壁半透明，边缘有 Fresnel 高光，管腔内部仍有深浅层次。
- 路径/导丝是细而亮的导航叠加，不压过血管本体。
- 相机沿导丝前端附近斜俯视，而不是工程式全树摊开。

当前 Godot 视图已有青蓝 Fresnel、bloom、环绕相机和路径线基础，但主要差距仍在几何平滑、透明层次、景深主次和真实风险叠加。

## 二、差距分析

### 2.1 几何质量

参考图血管表面连续、圆润，分叉融合自然，看不出三角面片。当前模型能看到明显低多边形折面、端口截断和分叉硬边，导致观感更像工程调试网格，而不是医学影像重建。

主要原因：

- 当前 GLB mesh 面片密度不足或导出时抽稀过度。
- Godot 导入/材质没有强制平滑法线。
- 分叉处来自原始表面网格，缺少渲染层面的平滑或重建。

### 2.2 材质与透明层次

参考图的血管是“边缘亮、中心透、远处分层变暗”。当前血管主体 cyan emission 偏满，亮度过于均匀，管腔内部与外壁层次不够，局部看起来像实心发光塑料。

需要从“整体发光”改为：

- 核心低透明度。
- 边缘 Fresnel 高亮。
- 深度/距离衰减。
- 远端分支降低 alpha 和 emission。
- 近相机片段淡出，避免血管贴镜头时糊成一团。

### 2.3 景深和主次关系

参考图中心主血管最清晰，后方分支明显虚化。当前所有分支同等清晰、同等亮，画面信息密度太高，缺少视觉焦点。

Godot 实现方向：

- 使用相机 DOF 或 shader 距离雾化制造远端虚化感。
- 以导丝前端或当前导航段作为焦点。
- 非焦点区域降低 alpha/emission，而不是简单隐藏。

### 2.4 相机视角

参考图是近景斜俯视，主血管从画面下方进入，向中心和上方延伸。当前视角更像全局工程总览，模型铺得太开。

当前已修正为导丝前端环绕，但仍需实机调参：

- 默认距离更近。
- 俯视角更高。
- 主血管在画面中形成纵向走势。
- 拖拽始终围绕导丝前端，而不是目标点或入口点。

### 2.5 路径与导丝叠加

参考图中的中心路径线细、亮、柔和，像导航 overlay。当前白色路径线偏粗偏硬，容易和血管主体竞争。

调整方向：

- 路径线半径减小。
- 提升 emission，但降低实体感。
- 已通过段继续暗化。
- 导丝尖端保留高对比，但不放大成遮挡视图的球。

### 2.6 风险区域显示

参考图有红色禁入区，但本项目当前没有真实危险区域建模。原则上：

**没有后端真实 `risk_regions` / SDF 风险场数据时，不显示红色禁入体积，不显示伪危险区。**

当前已经移除 60% 路径段 mock 禁入区。后续只能在真实风险数据接通后显示。

## 三、当前资产可行性评估

### 3.1 资产结论

当前后端和 Godot 侧已有多套血管资产，但它们对参考图效果的支撑能力不同：

| 资产 | 当前规模 | 结论 | 适合用途 |
|---|---:|---|---|
| `aorta_tree/visual.stl` | 约 4,032 顶点 / 7,584 面 | 不足以做高质量医学影像观感，低模折面明显 | 快速拓扑验证、分支导航调试 |
| `aorta_trunk/visual.stl` | 约 528 顶点 / 1,040 面 | 明显过低，只适合工程调试 | 物理/链路 smoke |
| `segment_part/visual.stl` | 约 89,966 顶点 / 180,000 面，`watertight=True` | 可以支撑真实表面凹凸和高质量玻璃血管渲染 | 近期视觉验收主资产 |
| `godot_client/assets/models/blood_vessels.glb` | 约 11 MB | 最有潜力接近参考图，来自 VPP 真实分割资产 | VPP 真实血管导航展示 |
| V-HACD `hull_*.stl` | 每块多为几百到几千字节 | 不适合渲染真实血管表面 | 碰撞近似 / 历史 MuJoCo 路线 |

因此，当前资产不是“完全做不到”，而是必须选对资产和导出管线。`aorta_tree` 低模版本无法直接达到参考图效果；`segment_part` 和 VPP `blood_vessels.glb` 才是应该用于视觉升级的主资产。

### 3.2 可实现的凹凸类型

当前资产可以支持两类凹凸：

1. **真实几何凹凸**  
   来自分割表面 mesh 本身，例如 `segment_part/visual.stl` 或 VPP `blood_vessels`。这种凹凸是真实血管分割、管径变化、分叉融合和表面采样带来的，可以用于医学视觉呈现。

2. **视觉微凹凸**  
   来自 shader normal noise / bump，只改变光照，不改变轮廓。它只能作为玻璃材质的微纹理，不得表达病灶、禁入区、狭窄或危险语义。

推荐组合：

- 大形态、分叉、管径变化：使用真实分割 mesh。
- 表面微纹理：可少量使用 shader bump，但强度要低。
- 风险/病灶/禁入区：必须来自后端真实 `risk_regions` 或 SDF/radius 异常检测，不得伪造。

### 3.3 当前效果差的直接原因

当前截图中的低模感主要来自：

- 运行时使用的 GLB 可能是 `aorta_tree.glb` 一类低面数资产。
- `tools/export_godot_assets.py` 默认将 VPP mesh 抽稀到 `--max-faces 120000`，性能友好但会损失细节。
- Godot 导入后没有专门做 smooth normals / 高质量视觉法线处理。
- 当前 shader 强调整体 cyan emission，真实表面起伏没有通过法线和透明层次充分显现。

### 3.4 资产改造建议

后续视觉管线应明确分离三类资产：

| 类型 | 文件建议 | 用途 | 要求 |
|---|---|---|---|
| 高质量视觉 mesh | `visual_high.glb` | Godot 渲染 | 高面数、平滑法线、保留真实表面细节 |
| 轻量预览 mesh | `visual_preview.glb` | 低配/调试 | 可抽稀，不能作为最终验收 |
| 碰撞/SDF 资产 | `collision_sdf` / `hull_*.stl` | Newton 物理碰撞 | 密封、稳定、性能优先，不追求视觉 |

`tools/export_godot_assets.py` 建议增加参数：

```text
--quality visual_high
--max-faces 300000
--no-decimate
--smooth-normals
--taubin-smooth-iter N
```

建议默认策略：

- `segment_part`：优先直接导出高质量 `visual.stl`，不抽稀或轻微抽稀。
- `case_001_vpp`：导出 `blood_vessels_visual_high.glb`，面数上限提高到 300k 或根据实机性能决定。
- `aorta_tree`：不作为最终视觉验收资产，除非重新由高质量分割/中心线半径重建。

### 3.5 验收判断

资产层验收应先于 shader 调参：

- 若血管近景仍能看到明显三角折面，优先换高质量 mesh，不继续调材质。
- 若分叉处是硬切面或断裂，优先检查源 mesh / 重建管线。
- 若血管几何已平滑但仍像塑料，再进入 Fresnel、bloom、景深调参。
- 若性能下降，优先做 LOD 或分视角加载，不回退到低模主资产。

## 四、改造原则

1. 真实优先  
   任何红色危险区、禁入区、风险体积必须来自后端真实数据，不允许用固定比例、固定球体、固定路径段伪造。

2. 渲染与物理解耦  
   血管视觉可以用高质量 GLB / shader / 后处理优化；碰撞仍按 Newton/SDF 物理轨道推进，两者不互相阻塞。

3. 主视角服务操作  
   3D 视图默认围绕导丝前端，而不是围绕全树中心、入口点或目标点。全局视图只作为复位/概览模式。

4. 不牺牲可读性  
   视觉升级不能让导丝、路径、目标点被 bloom 或透明血管淹没。

## 五、分阶段方案

### V0：立即约束与清理

状态：已执行。

- 移除 mock 红色风险体积。
- 清空固定 60% 路径段的 mock 禁入标红。
- 保留路径按曲率自然分色，但不把它解释为真实禁入区。
- 契约测试禁止 `_risk_volume` / `nogo_lo` 一类 mock 入口回归。

验收：

- 画面中不出现红色禁入体积。
- 路径不再因为固定百分比被强制标红。
- Godot CLI headless 加载无脚本错误。

### V1：血管几何平滑

目标：消除当前低模折面感。

任务：

- 检查 Godot 导入后的 GLB 是否保留 smooth normals。
- 在 `tools/export_godot_assets.py` 增加高质量渲染导出选项：
  - 禁止过度 decimate。
  - 重新计算平滑法线。
  - 必要时增加 subdivision / Taubin smoothing。
- 为不同用途分离资产：
  - `visual_high.glb`：Godot 渲染。
  - `collision_sdf`：Newton/SDF 碰撞。
  - `graph/routes`：导航拓扑。

验收：

- 主动脉主干近景不可见明显三角折面。
- 分支连接处无大块硬切面。
- 端口截断不在默认视角中形成强视觉焦点。

### V2：医学玻璃血管材质

目标：从“发光实心管”改为“半透明玻璃血管”。

任务：

- 重写 overview vessel shader 参数：
  - core alpha 降低。
  - Fresnel rim 强化。
  - emission 只集中在边缘。
  - 增加基于 camera distance 的 alpha/emission 衰减。
- follow/surgical shader 继续保留 tip proximity fade，但降低远端分支干扰。
- 调整 bloom：
  - rim bloom 可见。
  - 主体不整片过曝。

建议参数起点：

```text
rim_color       = (0.20, 0.68, 1.00)
core_color      = (0.02, 0.08, 0.18)
core_alpha      = 0.02 - 0.05
rim_power       = 2.0 - 2.6
glow            = 2.2 - 3.0
glow_intensity  = 0.8 - 1.1
```

验收：

- 主血管边缘亮，中心透明。
- 远处分支不抢主血管注意力。
- 无大面积 cyan 实心块。

### V3：景深与远端层次

目标：形成参考图的“中心清晰、背景分支虚化”。

任务：

- 以导丝前端 `_tip_world_last` 作为视觉焦点。
- 试验 Godot Camera3D DOF：
  - focus distance = camera 到导丝前端距离。
  - far blur 轻度开启。
- 如 DOF 性能或效果不稳定，改用 shader 距离雾化：
  - 离导丝越远，alpha/emission 越低。
  - 远端分支偏暗蓝。

验收：

- 导丝前端附近血管最清楚。
- 背景分支可辨认但不抢焦点。
- 拖拽环绕时焦点跟随导丝前端。

### V4：相机构图预设

目标：默认构图接近参考图，而不是全树工程视图；默认视角采用导丝运动跟踪相机，让血管路径前进方向始终朝向屏幕上方，形成类似地图导航的阅读方式。

任务：

- 新增相机预设：
  - `Guidewire Navigation Follow`：默认，导丝运动跟踪相机，类似地图导航视角。
  - `Clinical Orbit`：手动观察，围绕导丝前端近景斜俯视。
  - `Tree Overview`：全树概览，供复位/查看拓扑。
  - `Follow`：第三人称追尾。
  - `Endoscope`：腔内视角。
- 默认 `Guidewire Navigation Follow` 参数：
  - FOV 38-45。
  - 距离按局部血管半径/模型尺度自适应。
  - pitch 约 25-45 度俯视。
  - 相机目标点锁定导丝前端或前方 1-2 个路径采样点。
  - 画面上方向始终对齐当前规划路径前进方向。
  - 当路径切线进入弯道时，相机 yaw 随路径切线平滑转向，而不是保持世界坐标固定。
  - 导丝从画面下方或下中部进入，向画面上方推进，形成“导航地图”式阅读方向。
  - 路径未来 20-40mm 保持在画面上半区，已走路径淡出到画面下方。
  - 转弯时优先保持规划路径向上，允许血管模型在画面中旋转；避免让用户看到路径横向穿过屏幕。

相机姿态计算建议：

```text
focus = guidewire_tip_world 或 path.sample(arclen_tip + lookahead_mm)
forward = normalize(path.tangent(arclen_tip + lookahead_mm))
screen_up = forward
camera_right = normalize(cross(world_up, forward))
camera_dir = normalize(-forward * follow_back + world_up * follow_height)
camera_pos = focus - forward * follow_back + world_up * follow_height
look_at = focus + forward * lookahead_mm
roll/yaw 平滑时间常数：0.15 - 0.35s
tip 跟随平滑时间常数：0.08 - 0.20s
```

为避免弯道处画面突然翻转：

- 当相邻路径切线夹角大于阈值时使用 slerp 平滑相机朝向。
- 对局部路径切线做窗口平均，避免中心线采样噪声造成画面抖动。
- 使用最小 roll 变化策略，禁止相机在接近 180 度回头弯时瞬间翻滚。
- 若路径前方出现分叉，默认仍沿当前规划 route 对齐，不随非目标分支旋转。

验收：

- 首屏不再摊开整棵树。
- 主血管占据画面主体，分支作为背景层。
- 默认导丝推进方向在屏幕上保持向上。
- 导丝进入弯道时，视角随路径转弯，规划路径不会横向或倒向显示。
- 左键拖拽可临时进入 `Clinical Orbit` 手动观察，复位后回到导航跟踪相机。

### V5：路径与导丝叠加优化

目标：路径/导丝像导航 overlay，不像实体白管。

任务：

- 路径线半径降低 20%-40%。
- 路径 emission 提高但 alpha 降低，形成细亮线。
- 已走路径继续暗化。
- 导丝 tip 球缩小或改为小型发光点，避免遮挡。
- 目标点/入口点只在需要时显示，不长期压迫画面。

验收：

- 导丝位置清晰。
- 路径线不压过血管边缘。
- 目标/入口标记不会被误认为危险区。

### V6：真实风险区接入

目标：只有真实风险数据才显示红色/橙色区域。

前置：

- 后端提供明确空间字段，例如：

```json
{
  "risk_regions": [
    {
      "id": "stenosis_001",
      "level": "danger",
      "kind": "stenosis|collision|no_go|high_curvature",
      "center": [x, y, z],
      "radius": 0.008,
      "orientation": [qx, qy, qz, qw],
      "extent": [rx, ry, rz],
      "source": "sdf|annotation|planner"
    }
  ]
}
```

任务：

- 前端新增 `risk_region_renderer.gd`。
- 按 `level` 显示：
  - warning：橙色薄 halo。
  - danger：红色半透明体积。
- 每个区域必须有 `source` 字段；缺失则不显示。
- 不使用固定比例路径段作为风险区。

验收：

- 后端 `risk_regions=[]` 时画面无红色风险体积。
- 后端下发真实 region 时才显示。
- 显示位置与血管坐标一致。

## 六、优先级建议

| 优先级 | 事项 | 理由 |
|---|---|---|
| P0 | 保持无 mock 危险区 | 防止医学语义误导 |
| P0 | 几何平滑与高质量 GLB 导出 | 当前低模感最破坏参考图观感 |
| P1 | 玻璃血管 shader 调参 | 成本低，收益高 |
| P1 | 导丝前端近景相机预设 | 直接影响首屏是否像参考图 |
| P2 | 景深/距离雾化 | 提升医学影像层次 |
| P2 | 路径/导丝 overlay 精修 | 提升专业感 |
| P3 | 真实风险区 renderer | 等后端真实风险数据稳定后接入 |

## 七、验收清单

视觉验收：

- 血管近景无明显低模折面。
- 主血管边缘有蓝色 bloom，中心不过曝。
- 远端分支暗且柔，不抢焦点。
- 默认视角围绕导丝前端，而不是目标点/入口点/全树中心。
- 路径线细亮、导丝清楚、目标点不过度遮挡。
- 没有真实风险数据时，不出现红色禁入区或危险体积。

工程验收：

- Godot CLI headless 加载无脚本错误。
- 前端契约测试禁止 mock 风险区回归。
- 高质量视觉 GLB 与物理碰撞/SDF 资产分离。
- 切换模型后相机、材质、路径、导丝仍在同一坐标系。

## 八、当前代码关联点

主要修改入口：

- `godot_client/scripts/main_controller.gd`
  - `_make_fresnel_material`
  - `_setup_environment`
  - `_frame_camera`
  - `_set_orbit_focus`
  - `_guidewire_front_pose_from_batch`
- `godot_client/scripts/path_renderer.gd`
  - `path_radius_main`
  - `emission_energy`
  - `traversed_dim`
  - `set_risk_ranges`
- `godot_client/scripts/guidewire_renderer.gd`
  - `wire_radius_overview`
  - `tip_radius_overview`
  - tip/root marker material
- `tools/export_godot_assets.py`
  - 高质量视觉 mesh 导出
  - smooth normals / decimation / smoothing 策略

后端真实风险区前置：

- `services/navigation_engine.py`
  - `NavigationState.risk_regions`
  - `_risk_regions`
- `services/websocket_handler.py`
  - `state_batch.safety.risk_regions`

## 九、注意事项

- 不要把曲率高的路径段直接渲染成禁入区。曲率可用于路径颜色提示，但红色体积必须来自真实空间风险。
- 不要为了视觉效果改变物理碰撞资产；渲染 mesh 与 SDF/碰撞数据应分离。
- 不要让 bloom 掩盖导丝尖端位置，导丝是导航主对象。
- 不要把文字标注、箭头和医学风险语义混入本阶段；先把血管本体、相机和叠加层做好。
