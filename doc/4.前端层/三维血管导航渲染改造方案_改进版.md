...
- 当前验收：未完成。当前 `state_batch.safety` 仅包含 `status`、`wall_distance`、`curvature`、`speed`、`risk_score`、`risk_regions`，尚无 `guidewire_mechanics`、`safety_level`、`stop_required`、`reason_codes`、`timestamp_ms` 等契约字段。
- 当前验收：前端 `path_renderer.gd` 仍以曲率提示渐变和已走段暗化为主，没有 `set_visual_state(level, score, stale)`、迟滞/minimum hold、stale 灰色未知态或 stop 控制与颜色动画解耦的可验实现。
