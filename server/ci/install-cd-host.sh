...
install -d -o root -g root -m 0755 /opt/cathsim/registry/data
install -d -o root -g root -m 0700 /opt/cathsim/registry/certs
...
install -o root -g root -m 0644 \
  "$SOURCE_DIR/compose.registry.yaml" \
  /opt/cathsim/compose/compose.registry.yaml
...
if [[ ! -f /opt/cathsim/registry/certs/registry.crt ]]; then
  openssl req -x509 -newkey rsa:4096 -sha256 -nodes -days 3650 \
    -subj "/CN=192.168.1.107" \
    -addext "subjectAltName=IP:192.168.1.107" \
    -keyout /opt/cathsim/registry/certs/registry.key \
    -out /opt/cathsim/registry/certs/registry.crt
  chmod 0600 /opt/cathsim/registry/certs/registry.key
  chmod 0644 /opt/cathsim/registry/certs/registry.crt
fi
install -d -o root -g root -m 0755 \
  /etc/docker/certs.d/192.168.1.107:5001
install -o root -g root -m 0644 \
  /opt/cathsim/registry/certs/registry.crt \
  /etc/docker/certs.d/192.168.1.107:5001/ca.crt
docker compose -f /opt/cathsim/compose/compose.registry.yaml up -d
